...
if($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
  ...
  {
    if(($_POST['ajtok'] == $_SESSION['ajtoken']) && (isset($_POST['charid'])) && (isset($_POST['state'])) && ($_SESSION['isAdmin'] == True)) {
      $qry = DB::getConnection();
      $sql = "SELECT * FROM recruiters WHERE characterID=".$_POST['charid'];
      $result = $qry->query($sql);
      if ($result->num_rows) {
        if ($stmt = $qry->prepare("UPDATE recruiters SET notify=? WHERE characterID=?")) {
          $stmt->bind_param('ii', $state, $charid);
          $state = $_POST['state'];
          $charid = $_POST['charid'];
          $stmt->execute();
          if ($stmt->errno) {
